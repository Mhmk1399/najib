...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    return jsonResponse(await commerceOperationsService.getCart((await context.params).id), { cache: "no-store" });
  } catch (error) {
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    return jsonResponse(
      await commerceOperationsService.actOnCart((await context.params).id, await request.json(), staff.id),
      { cache: "no-store" },
    );
  } catch (error) {
    return jsonError(error);
