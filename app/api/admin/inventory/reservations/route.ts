...
export async function GET(request: Request) {
  try {
    ...
    return jsonResponse(await inventoryService.listReservations(query), { cache: "no-store" });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return jsonResponse(await inventoryService.reserve(await request.json(), staff.id), {
      status: 201,
      cache: "no-store",
    });
  } catch (error) {
