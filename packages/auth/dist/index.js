...
export function resolveStaffPermissions(roles, additions = []) {
    return [...new Set([...roles.flatMap((role) => ROLE_PERMISSIONS[role]), ...additions])];
}
export function createStaffAccessToken(input, secret, ttlSeconds, nowSeconds = Math.floor(Date.now() / 1000)) {
    if (Buffer.byteLength(secret) < 32)
        throw new Error("Access token secret must contain at least 32 bytes");
    const header = encode(JSON.stringify({ alg: "HS256", typ: "JWT" }));
    const payload = encode(JSON.stringify({ ...input, iss: ACCESS_TOKEN_ISSUER, aud: ACCESS_TOKEN_AUDIENCE, typ: "staff_access", iat: nowSeconds, exp: nowSeconds + ttlSeconds }));
    const unsigned = `${header}.${payload}`;
    return `${unsigned}.${sign(unsigned, secret)}`;
}
export function verifyStaffAccessToken(token, secret, nowSeconds = Math.floor(Date.now() / 1000)) {
    try {
...
export function hasEveryPermission(claims, required) {
