...
function strokeStyle(
  ...
  const baseDelay = tone === "ghost" ? 0.04 : tone === "main" ? 0.42 : 0.88;
  const duration = tone === "ghost" ? 1.25 : tone === "main" ? 1.55 : 1.2;
  const stagger = (index % 18) * 0.026 + Math.floor(index / 18) * 0.035;
  ...
  return (
    ...
    >
      ...
      <g filter={`url(#${lineShadowId})`}>
        <g filter={`url(#${pencilFilterId})`} className="najib-loader-pencil-jitter">
          ...
          <g className="najib-loader-pass najib-loader-pass--ghost">
            {NATURAL_GHOST_PATHS.map((d, index) => (
              <SketchStroke key={`ghost-${index}`} d={d} tone="ghost" index={index} />
            ))}
          </g>
          ...
          <g className="najib-loader-pass najib-loader-pass--main">
            {NATURAL_MAIN_PATHS.map((d, index) => (
              <SketchStroke key={`main-${index}`} d={d} tone="main" index={index} />
            ))}
          </g>
          ...
          <g className="najib-loader-pass najib-loader-pass--detail">
            {NATURAL_DETAIL_PATHS.map((d, index) => (
              <SketchStroke key={`detail-${index}`} d={d} tone="detail" index={index} />
            ))}
          </g>
        </g>
      </g>
...
export function BrandSketchLoader({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setRendered(false), closeDelayMs);
  ...
  const portalStyle = useMemo<CSSProperties>(() => ({ zIndex }), [zIndex]);
  ...
  return createPortal(
    <div
      ...
      aria-label={label}
      style={portalStyle}
      className={`fixed inset-0 grid place-items-center z-10000000000000 overflow-hidden transition-opacity duration-300 ease-out ${
        open ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"
      } ${className}`}
    >
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-[#fbf8f2]/[0.91] backdrop-blur-[3px]"
      />

      <div className="relative z-10 flex w-[min(86vw,360px)] flex-col items-center sm:w-[380px] md:w-[405px] lg:w-[430px]">
        <div className="w-[min(72vw,268px)] sm:w-[286px] md:w-[306px] lg:w-[324px]">
          <CoutureSketch uid={uid} />
        </div>

        <div
          aria-hidden="true"
          className="relative -mt-1 h-px w-[min(58vw,224px)] overflow-hidden bg-[#2f2b27]/15 sm:w-[236px] md:w-[252px]"
        >
          <span className="najib-loader-progress absolute inset-y-0 left-0 block w-[38%] bg-[#26231f]" />
        </div>

        <div className="mt-7 text-center sm:mt-8">
          <p
            className="whitespace-nowrap text-[clamp(1.45rem,6vw,2.35rem)] font-normal leading-none tracking-[0.32em] text-[#24211e] sm:tracking-[0.38em]"
            style={{
              fontFamily:
          ...
          <span className="mx-auto mt-5 block h-px w-9 bg-[#3c3833]/70" />

          <p className="mt-4 whitespace-nowrap text-[7px] font-medium uppercase tracking-[0.42em] text-[#615c55] sm:text-[8px] sm:tracking-[0.48em]">
            {tagline}
          </p>
        </div>
      </div>
      ...
      <style>{`
        .najib-loader-line {
          ...
          stroke-dasharray: 1;
          stroke-dashoffset: 1;
          animation: najib-loader-draw var(--najib-draw-duration, 1.45s)
            cubic-bezier(.22, 1, .36, 1) var(--najib-draw-delay, 0s) forwards;
        }

        .najib-loader-line--ghost {
          stroke: rgba(82, 75, 67, .39);
          stroke-width: .82;
        }

        .najib-loader-line--main {
          stroke: rgba(31, 29, 27, .92);
          stroke-width: 1.48;
        }

        .najib-loader-line--detail {
          stroke: rgba(49, 45, 41, .70);
          stroke-width: 1.02;
        }

        .najib-loader-line--under {
          stroke: rgba(82, 73, 64, .22);
          stroke-width: 2.15;
        }
        ...
        .najib-loader-pass--ghost { opacity: .82; }
        .najib-loader-pass--main { opacity: 1; }
        .najib-loader-pass--detail { opacity: .94; }

        .najib-loader-pencil-jitter {
          transform-origin: 51% 45%;
          animation: najib-loader-jitter 4.4s steps(2, end) 1.65s infinite;
        }

        .najib-loader-volume-pass {
          opacity: 0;
          animation: najib-loader-volume-in 1.2s ease 1.15s forwards;
        }

        .najib-loader-ground-shadow {
          transform-origin: center;
          animation: najib-loader-shadow-in 1.05s ease .72s both,
            najib-loader-shadow-breathe 3.6s ease-in-out 2s infinite;
        }

        .najib-loader-progress {
          transform-origin: left center;
          animation: najib-loader-sweep 1.65s cubic-bezier(.64, 0, .36, 1)
            infinite alternate;
        ...
        @keyframes najib-loader-jitter {
          ...
          25% { transform: translate3d(.15px, -.10px, 0) rotate(.012deg); }
          50% { transform: translate3d(-.13px, .12px, 0) rotate(-.011deg); }
          75% { transform: translate3d(.09px, .07px, 0) rotate(.009deg); }
        }
        ...
        @keyframes najib-loader-shadow-breathe {
          0%, 100% { opacity: .82; transform: scaleX(.96); }
          50% { opacity: 1; transform: scaleX(1.04); }
        }

        @keyframes najib-loader-sweep {
          0% { transform: translateX(-112%) scaleX(.34); opacity: .45; }
          45% { opacity: 1; }
          100% { transform: translateX(265%) scaleX(1.15); opacity: .72; }
        }

        @media (max-width: 420px) {
          .najib-loader-pencil-jitter { animation-duration: 4.9s; }
        }
