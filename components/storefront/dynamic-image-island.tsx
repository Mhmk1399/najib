...
async function fetchJson<T>(input: RequestInfo | URL, init?: RequestInit) {
  ...
  if (!response.ok) {
    ...
    throw new Error(body?.error ?? "Image suggestions could not be loaded.");
  }
...
export function DynamicImageIsland() {
  ...
  const copy = imageStoryCopy[locale].island;
  ...
  const activeTitle = activeStory
    ...
    : copy.assistantTitle;
  const activeKicker = activeStory ? copy.relatedKicker : copy.searchKicker;
  ...
  function prefetchProduct(product: StoryProduct) {
    ...
    void queryClient.prefetchQuery({
      queryKey: ["storefront", "product-detail", locale, product.slug],
      queryFn: ({ signal }) =>
  ...
  function revealProduct(product: StoryProduct) {
    ...
    if (
      ...
      router.push(localizedHref(product.href, locale));
      closeIsland();
      return;
    }
                ...
                <div className="grid gap-2.5">
                  ...
                  >
                    ...
                    <input
                      ...
                      autoComplete="off"
                      className="min-w-0 flex-1 bg-transparent text-start text-[10.5px] leading-6 text-white outline-none placeholder:text-white/72"
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 px-1">
                    ...
                    >
                      {text(configuredCtaLabel, locale, copy.allProducts)}
                      <ArrowLeft className="size-3" aria-hidden="true" />
                    </Link>
                  </div>
                </div>
