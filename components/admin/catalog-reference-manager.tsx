...
const resources: Array<{
...
}> = [
  {
    id: "collections",
    label: "مجموعه‌ها",
    description: "فصل‌ها و کمپین‌های فروش",
    icon: Layers3,
  },
  {
    id: "colors",
    label: "رنگ‌ها",
    description: "نام، خانواده و نمونه رنگ",
    icon: Palette,
  },
  {
    id: "size-groups",
    label: "گروه‌های سایز",
    description: "ساختارهای اندازه‌گیری",
    icon: Shapes,
  },
  { id: "sizes", label: "سایزها", description: "سایزهای هر گروه", icon: Ruler },
  {
    id: "variants",
    label: "تنوع محصولات",
    description: "ترکیب محصول، رنگ و سایز",
    icon: Tags,
  },
];
...
function recordLabel(record: ReferenceRecord) {
  return record.name
    ? fa(record.name)
...
function requiredLocalized(value: LocalizedText | undefined, label: string) {
  return value?.fa.trim() && value.en.trim() && value.ar.trim()
    ? null
    : `${label} باید در هر سه زبان تکمیل شود.`;
}
...
async function all(resource: string): Promise<ReferenceRecord[]> {
  ...
  let page = 1;
  while (true) {
    const data = await api<ListResponse<ReferenceRecord>>(
      `/api/catalog/${resource}?page=${page}&limit=100`,
    );
...
function options(records: ReferenceRecord[]) {
  return records.map((record) => ({
    value: record._id,
    label: recordLabel(record),
    description: record.slug ?? record.sku ?? record.code,
  }));
}
...
function schemaFor(
  ...
  if (resource !== "variants")
    fields.push(
      ...localizedFields("name", "نام").map((field) => ({
        ...field,
        required: true,
      })),
    );
  if (resource === "collections" || resource === "colors")
    fields.push({
      kind: "input",
      name: "slug",
      label: "شناسه URL",
  ...
  if (resource === "colors")
    fields.push(
      ...localizedFields("family", "خانواده رنگ").map((field) => ({
        ...field,
        required: true,
      })),
      {
      ...
      },
      {
        ...
        inputType: "url",
        name: "swatchImageUrl",
        label: "URL تصویر نمونه",
        dir: "ltr",
      },
    );
  if (resource === "size-groups")
    fields.push({
      kind: "input",
      name: "code",
      label: "کد گروه",
  ...
  return {
    ...
    validate: (values) => {
      ...
      if (resource !== "variants") {
        const error = requiredLocalized(values.name, "نام");
        if (error) errors["name.fa"] = error;
      }
      if (
        (resource === "collections" || resource === "colors") &&
        !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(values.slug)
      )
        errors.slug = "شناسه URL باید انگلیسی، کوچک و با خط تیره باشد.";
      if (resource === "colors") {
        const error = requiredLocalized(values.family, "خانواده رنگ");
        if (error) errors["family.fa"] = error;
        if (values.hex && !/^#[0-9A-Fa-f]{6}$/.test(values.hex))
          errors.hex = "کد رنگ باید مانند #1C1C1C باشد.";
      }
      if (resource === "sizes" && !values.sizeGroupId)
        errors.sizeGroupId = "گروه سایز را انتخاب کنید.";
      if (resource === "variants") {
        if (!values.productId) errors.productId = "محصول را انتخاب کنید.";
        if (!values.colorId) errors.colorId = "رنگ را انتخاب کنید.";
        if (!values.sizeId) errors.sizeId = "سایز را انتخاب کنید.";
        if (!values.sku.trim()) errors.sku = "SKU الزامی است.";
      }
      return errors;
    },
  };
...
function payload(resource: Resource, value: FormValues) {
  ...
  if (resource === "colors")
    return {
      name: trimLocalized(value.name),
      slug: slugify(value.slug),
      family: trimLocalized(value.family),
      hex: value.hex.trim() || undefined,
      swatchImageUrl: value.swatchImageUrl.trim() || undefined,
      isActive: value.isActive,
      sortOrder: Number(value.sortOrder) || 0,
    };
  if (resource === "size-groups")
...
export function CatalogReferenceManager({ canWrite }: { canWrite: boolean }) {
  ...
  const groups = useQuery({
    queryKey: ["catalog", "all-size-groups"],
    queryFn: () => all("size-groups"),
    enabled: resource === "sizes",
  });
  ...
  const current =
    resources.find((item) => item.id === resource) ?? resources[0];
  const refMaps = useMemo(
    () => ({
      groups: new Map((groups.data ?? []).map((r) => [r._id, recordLabel(r)])),
      products: new Map(
        (products.data ?? []).map((r) => [r._id, recordLabel(r)]),
      ),
      colors: new Map((colors.data ?? []).map((r) => [r._id, recordLabel(r)])),
      sizes: new Map((sizes.data ?? []).map((r) => [r._id, recordLabel(r)])),
    }),
    [groups.data, products.data, colors.data, sizes.data],
  );
  const columns = useMemo<DynamicColumn<ReferenceRecord>[]>(() => {
    const first: DynamicColumn<ReferenceRecord> = {
      id: "identity",
      label: resource === "variants" ? "تنوع" : "عنوان",
      minWidth: 220,
      sticky: "start",
      lockVisibility: true,
      cell: ({ record }) => (
        <span>
          <strong className="block text-[11px]">{recordLabel(record)}</strong>
          <small
            dir="ltr"
            className="mt-1 block text-left text-[9px] text-[var(--adt-muted)]"
          >
            {record.slug ?? record.sku ?? record.code ?? "—"}
          </small>
        </span>
      ),
      ...
      result.push(
        {
          id: "family",
          label: "خانواده",
          cell: ({ record }) => (record.family ? fa(record.family) : "—"),
        },
        {
          id: "hex",
          label: "رنگ",
          cell: ({ record }) => (
            <span className="inline-flex items-center gap-2" dir="ltr">
              <i
                className="size-4 border border-[var(--adt-border)]"
                style={{ background: record.hex || "transparent" }}
              />
              {record.hex || "—"}
            </span>
          ),
    ...
    if (resource === "sizes")
      result.push({
        ...
        label: "گروه",
        cell: ({ record }) =>
          refMaps.groups.get(record.sizeGroupId ?? "") ?? "—",
      });
    if (resource === "variants")
      result.push(
        {
          ...
          label: "محصول",
          cell: ({ record }) =>
            refMaps.products.get(record.productId ?? "") ?? "—",
        },
        {
          id: "color",
          label: "رنگ",
          cell: ({ record }) => refMaps.colors.get(record.colorId ?? "") ?? "—",
        },
        {
          id: "size",
          label: "سایز",
    ...
    result.push({
      ...
      cell: ({ record }) => (
        <span
          className={
            record.isActive
              ? "text-[var(--adt-success)]"
              : "text-[var(--adt-warning)]"
          }
        >
  ...
  const save = (id: string | null, values: FormValues) =>
    api<ReferenceRecord>(`/api/catalog/${resource}${id ? `/${id}` : ""}`, {
      method: id ? "PATCH" : "POST",
      body: JSON.stringify(payload(resource, values)),
    });

  return (
    <div className="min-w-0 space-y-3 p-3 text-[var(--adt-text)] sm:p-4 lg:p-5">
      <CatalogSectionNav />
      <section className="border border-[var(--adt-border)] bg-[var(--adt-surface)] p-3 text-[var(--adt-text)]">
        <h1 className="px-1 text-[16px] font-bold">اطلاعات پایه کاتالوگ</h1>
        <p className="mt-1 px-1 text-[11px] leading-6 text-[var(--adt-muted)]">
          مجموعه‌ها، رنگ‌ها، سایزها و تنوع‌های قابل انتخاب محصول را از این بخش
          مدیریت کنید.
        </p>
        <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">
          {resources.map(({ id, label, description, icon: Icon }) => (
            <DataButton
              key={id}
              tone={resource === id ? "danger" : "ghost"}
              size="md"
              icon={<Icon size={15} />}
              onClick={() => setResource(id)}
              aria-pressed={resource === id}
            >
              <span className="text-right">
                <strong className="block text-[11px]">{label}</strong>
                <small className="mt-0.5 block text-[8px] opacity-65">
                  {description}
                </small>
              </span>
            </DataButton>
      ...
      <DynamicDataTable<ReferenceRecord, FormValues, FormValues, Filters>
        ...
        source={{
          ...
          fetchPage: async ({ page, pageSize, search, filters, signal }) => {
            ...
            } satisfies DynamicTableResult<ReferenceRecord>;
          },
        }}
        columns={columns}
        getRowId={(record) => record._id}
        getRowLabel={recordLabel}
        search={{ placeholder: "جستجو در این بخش…" }}
        filters={[
          {
            id: "isActive",
            kind: "select",
        ...
        mobile={{
          title: recordLabel,
          badge: (record) => (record.isActive ? "فعال" : "غیرفعال"),
          maxFields: 4,
        }}
        crud={{
          create: {
            enabled: canWrite,
            label: `${current.label} جدید`,
            title: `ساخت ${current.label}`,
            schema,
            initialValues: emptyForm,
            mutationFn: ({ values }) => save(null, values),
            onSuccess: () => {
              invalidate();
              toast.success("رکورد ساخته شد");
            },
            mapError: (error) => ({
              message:
            ...
            fields: [
              {
                id: "title",
                label: "عنوان",
                render: ({ record }) => recordLabel(record),
              },
              {
                id: "code",
                label: "شناسه",
              ...
              },
              {
                id: "status",
                label: "وضعیت",
                render: ({ record }) => (record.isActive ? "فعال" : "غیرفعال"),
              },
            ],
          },
          delete:
        ...
        emptyState={{
          ...
          description: canWrite
            ? "برای شروع، اولین رکورد را بسازید."
