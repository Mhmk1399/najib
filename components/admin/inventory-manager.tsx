...
type Variant = { _id: string; sku: string; isActive: boolean };
type Balance = {
  _id: string;
  variantId: RefValue;
  locationId: RefValue;
...
function asOptions(items: Array<MasterRecord | Variant>): DataSelectOption[] {
  return items.map((item) => ({
    value: item._id,
    label: "sku" in item ? item.sku : fa(item.name),
    description: "sku" in item ? item._id : item.code,
    disabled: !item.isActive,
  }));
}
function emptyMaster(): MasterForm {
  return { code: "", name: emptyLocalizedText(), countryCode: "IR", cityId: "", poolId: "", storeId: "", address: emptyLocalizedText(), type: "warehouse", isActive: true };
}
...
export function InventoryManager({ canWrite }: { canWrite: boolean }) {
  ...
  const [dialog, setDialog] = useState<MutationKind | null>(null);
  ...
  const locations = useQuery({ queryKey: ["inventory", "refs", "locations"], queryFn: () => all<MasterRecord>("/api/admin/inventory/locations") });
  const variants = useQuery({ queryKey: ["inventory", "refs", "variants"], queryFn: () => all<Variant>("/api/catalog/variants") });
  const references = useMemo(() => ({
    cities: asOptions(cities.data ?? []), stores: asOptions(stores.data ?? []), pools: asOptions(pools.data ?? []), locations: asOptions(locations.data ?? []), variants: asOptions(variants.data ?? []),
  }), [cities.data, stores.data, pools.data, locations.data, variants.data]);
  const referencesLoading = [cities, stores, pools, locations, variants].some((query) => query.isLoading);
  const referencesError = [cities, stores, pools, locations, variants].some((query) => query.isError);
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          {canWrite ? (
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
              <DataButton tone="primary" icon={<Plus size={15} />} onClick={() => setDialog("adjustment")}>اصلاح موجودی</DataButton>
              <DataButton icon={<ArrowLeftRight size={15} />} onClick={() => setDialog("transfer")}>انتقال موجودی</DataButton>
              <DataButton icon={<ShieldCheck size={15} />} onClick={() => setDialog("reservation")}>ایجاد رزرو</DataButton>
            </div>
          ) : null}
        </div>
      </section>
...
function masterSchema(resource: MasterResource, refs: Record<string, DataSelectOption[]>): DynamicFormSchema<MasterForm> {
  ...
  if (resource === "stores") fields.push({ kind: "select", name: "cityId", label: "شهر", options: refs.cities, searchable: true, required: true }, ...localizedFields("address", "نشانی"));
  ...
  return { fields, validate: (values) => { const errors: Record<string, string> = {}; if (!/^[A-Za-z0-9_-]{2,40}$/.test(values.code.trim())) errors.code = "کد باید ۲ تا ۴۰ نویسه انگلیسی باشد."; if (!values.name.fa.trim() || !values.name.en.trim() || !values.name.ar.trim()) errors["name.fa"] = "نام باید در هر سه زبان تکمیل شود."; if (resource === "cities" && !/^[A-Za-z]{2}$/.test(values.countryCode)) errors.countryCode = "کد کشور باید دو حرف باشد."; if (["stores", "locations"].includes(resource) && !values.cityId) errors.cityId = "شهر را انتخاب کنید."; if (resource === "locations" && !values.poolId) errors.poolId = "استخر موجودی را انتخاب کنید."; if (resource === "locations" && values.type === "store" && !values.storeId) errors.storeId = "برای این نوع، فروشگاه الزامی است."; return errors; } };
}
function masterToForm(record: MasterRecord): MasterForm { return { ...emptyMaster(), ...record, cityId: refId(record.cityId), poolId: refId(record.poolId), storeId: refId(record.storeId), address: record.address ?? emptyLocalizedText() }; }
function masterPayload(resource: MasterResource, values: MasterForm) {
  ...
  if (resource === "stores") return { ...base, cityId: values.cityId, address: trimLocalized(values.address) };
...
function InventoryActionModal({ kind, onClose, refs, referencesLoading, referencesError, onRetryReferences, onSuccess }: { kind: MutationKind | null; onClose: () => void; refs: Record<string, DataSelectOption[]>; referencesLoading: boolean; referencesError: boolean; onRetryReferences: () => void; onSuccess: () => void }) {
  ...
  const referencesEmpty = !referencesLoading && !referencesError && (refs.variants.length === 0 || refs.locations.length === 0);
