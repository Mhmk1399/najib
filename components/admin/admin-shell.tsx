...
function Brand({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`brand ${compact ? "brand--compact" : ""}`}>
      ...
      {!compact && (
        <div>
          <strong>NAJIBZADEH</strong>
          <span>عملیات آتلیه</span>
        </div>
      )}
    </div>
  );
...
function Navigation({
  ...
  return (
    <nav aria-label="ناوبری اصلی" className="sidebar-nav">
      {groups.map((group) => (
            ...
            .map(({ label, icon: Icon, count, href }) => {
              ...
              return href ? (
                ...
                <button
                  ...
                  disabled
                  title={collapsed ? `${label} — به‌زودی` : "به‌زودی"}
                >
                  {content}
                </button>
              );
...
export function AdminShell({
  ...
  const staffProfile = staff ?? {
    ...
    lastName: "تیم",
    displayRole: "همکار",
    permissions: navigation.map((item) => item.permission),
  };
  const permissions = staffProfile.permissions ?? [];
  const fullName = `${staffProfile.firstName} ${staffProfile.lastName}`.trim();
  const initials =
    `${staffProfile.firstName.charAt(0)}${staffProfile.lastName.charAt(0)}`.toUpperCase() ||
    "ST";
  ...
  useEffect(() => {
    ...
    if (mobileOpen)
      setTimeout(
        () =>
          document
            .querySelector<HTMLButtonElement>(".mobile-sheet .nav-item")
            ?.focus(),
        30,
      );
  ...
  const active = pathname.startsWith("/admin/catalog") ? "کاتالوگ" : "نمای کلی";
      ...
      <div className="workspace">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            >
              <Search size={17} />
              <span>جست‌وجو در داشبورد</span>
              <kbd>⌘ K</kbd>
            </button>
            <button className="icon-button notification" aria-label="اعلان‌ها">
              <Bell size={18} />
              <i />
            </button>
            <button
              ...
              onClick={toggleTheme}
              aria-label={darkTheme ? "پوسته روشن" : "پوسته تیره"}
            >
            ...
            <div className="profile-wrap" ref={profileRef}>
              ...
              >
                ...
                <div>
                  ...
                  <small>{staffProfile.displayRole}</small>
                </div>
                <ChevronDown
              ...
              {profileOpen && (
                ...
                >
                  ...
                  <div className="profile-menu-role">
                    ...
                    <span>واردشده به‌عنوان همکار</span>
                  </div>
      ...
      {mobileOpen && (
        <>
          <button
            ...
            aria-label="بستن منو"
          />
          <aside className="mobile-sheet" aria-label="منوی موبایل">
        ...
        >
          <section className="command-panel">
            <div className="command-input">
              ...
              <input
                ref={searchRef}
                placeholder="جست‌وجوی سفارش، مشتری یا محصول…"
                aria-label="جست‌وجو"
              />
